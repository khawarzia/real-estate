    ...
    def clean_email(self):
        ...
        for i in emails:
            if email==i.email:
                ...
                raise forms.ValidationError('This email already exists, check your email') 
...
class profileInformForm(forms.ModelForm):
    class Meta:
        model = profileModel
        exclude=['user','approve','Teacher_or_Parent']


#User Extra Information During registration ................
class profileInformFormRegister(forms.ModelForm):
    class Meta:
        model = profileModel
        fields = [
            'phone',
    ...
    def clean_phone(self):
        if self.cleaned_data.get('phone') is not None:
            try:
                int(self.cleaned_data.get('phone'), base=10)
            except:
    ...
    def clean_occupation(self):
        return self.cleaned_data.get('occupation')
